...
async function saveCustomized(req, res) {
    try {
        ...
        const existingCustomizedOrder = user.customizedBurgers.find(
            (order) => order.name === burgerName
        );
        if (existingCustomizedOrder) {
            return res
                .status(409)
                .json({ message: "Burger already customized" });
        }

        const newCustomizedOrder = {
            name: burgerName,
